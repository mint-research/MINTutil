...
$script:GlobalConfigFile = Join-Path -Path $script:BasePath -ChildPath "config/global/global.config.json"
...
function Scan-Modules {
    try {
        ...
        $moduleDirs = Get-ChildItem -Path $script:MetaPath -Directory | Where-Object { $_.Name -ne "global" }
