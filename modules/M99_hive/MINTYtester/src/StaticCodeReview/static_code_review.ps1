...
.EXAMPLE
    .\tests\static_code_review\static_code_review.ps1 -Path "C:\MINTutil" -OutputPath "C:\Reports\review.md"
.NOTES
    Version: 1.0
#>
...
function Initialize-Review {
    ...
    if ([string]::IsNullOrEmpty($OutputPath)) {
        $reportsDir = Join-Path -Path $script:BasePath -ChildPath "tests\static_code_review\reports"
...
function Add-Problem {
    ...
    if ($AutoCorrectable) {
        ...
        # Automatische Korrektur anwenden, wenn gewünscht und möglich
        if (-not [string]::IsNullOrEmpty($CurrentCode) -and -not [string]::IsNullOrEmpty($CorrectedCode)) {
            Apply-AutoCorrection -FilePath $FilePath -CurrentCode $CurrentCode -CorrectedCode $CorrectedCode
        }
    }
}
...
<#
.SYNOPSIS
    Kurzbeschreibung der Funktion Start-ArchitectureReview.
.DESCRIPTION
    Detaillierte Beschreibung der Funktion Start-ArchitectureReview.
.PARAMETER Parameter1
    Beschreibung des ersten Parameters.
.EXAMPLE
    Start-ArchitectureReview -Parameter1 Wert
#>
...
<#
.SYNOPSIS
    Kurzbeschreibung der Funktion Start-ModuleReview.
.DESCRIPTION
    Detaillierte Beschreibung der Funktion Start-ModuleReview.
.PARAMETER Parameter1
    Beschreibung des ersten Parameters.
.EXAMPLE
    Start-ModuleReview -Parameter1 Wert
#>
...
<#
.SYNOPSIS
    Kurzbeschreibung der Funktion Start-FunctionReview.
.DESCRIPTION
    Detaillierte Beschreibung der Funktion Start-FunctionReview.
.PARAMETER Parameter1
    Beschreibung des ersten Parameters.
.EXAMPLE
    Start-FunctionReview -Parameter1 Wert
#>
